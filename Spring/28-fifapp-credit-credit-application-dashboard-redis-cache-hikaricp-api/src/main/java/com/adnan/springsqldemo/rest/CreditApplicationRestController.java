...
public class CreditApplicationRestController {
  ...
  @GetMapping
  public List<CreditApplicationResponse> getAllCreditApplications(@RequestParam(value = "status", required = false) String status) {
  ...
  public CreditApplicationDashboardResponse getDashboard(@RequestParam(required = false) Long branchId) {
    return creditApplicationService.getDashboard(branchId);
  }
}
